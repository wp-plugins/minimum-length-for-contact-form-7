<?php
function wpcf7_min_textarea_init()
{
	if(function_exists('wpcf7_add_shortcode'))
	{
		wpcf7_add_shortcode( 'textarea', 'wpcf7_min_textarea_shortcode_handler', true );
		wpcf7_add_shortcode( 'textarea*', 'wpcf7_min_textarea_shortcode_handler', true );
	}	
	add_filter( 'wpcf7_validate_textarea', 'wpcf7_min_textarea_validation_filter', 10, 2 );
	add_filter( 'wpcf7_validate_textarea*', 'wpcf7_min_textarea_validation_filter', 10, 2 );
}

add_action( 'init', 'wpcf7_min_textarea_init' );

function wpcf7_min_textarea_shortcode_handler( $tag ) {
...
function wpcf7_min_textarea_validation_filter( $result, $tag ) {
	$tag = new WPCF7_Shortcode( $tag );
	...
	if (strlen($value) < $minlength[0] && ! isset($result['reason'][$name])) {
			$result['valid'] = false;
...
add_filter( 'wpcf7_messages', 'wpcf7_min_textarea_messages' );

function wpcf7_min_textarea_messages( $messages ) 
{
